...
var usageLogInsertArgTypes = [...]string{
	...
	"integer",     // original_cache_creation_tokens
...
func (r *usageLogRepository) createSingle(ctx context.Context, sqlq sqlExecutor, log *service.UsageLog) (bool, error) {
	...
	query := `
		INSERT INTO usage_logs (
			...
			original_cache_creation_tokens,
			image_output_tokens,
			image_output_cost,
			input_cost,
			output_cost,
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		OriginalCacheCreationTokens:    originalCacheCreation,
